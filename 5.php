...
<h1>PETTY CASH ADVANCE VALIDATION</h1>
<br><br>
...
<form action="uploads/5u.php" method="post">
...
<script>
// Headers
var headers = ["Location", "Advance Voucher no", "Reason for the Advance", "Has the advance voucher been aproved by the authorized party","Has the ERP been updated","Advance issue date","Subsequent settlement date","Delay to settlement"];
...
function loadData() {
  ...
  xhr.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
     ...
     for (let i = 0; i < 5; i++) {
  for (let j = 0; j < 8; j++) {
    document.getElementById((i * 8) + j + 1).value = data[i][j];
  }
}

    }
  };
  xhr.open("GET", "get/getdata4.php?date=" + site + "&site=" + site1, true);
  xhr.send();
}
