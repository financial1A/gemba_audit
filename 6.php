...
<h1>SALES VALIDATION - CASH SALES- CASH PAID OR TEMPORY CREDIT</h1>
<br><br>
...
<form action="uploads/6u.php" method="post">
...
<script>
// Headers
var headers = ["Location", "Cash Invoice no.", "Customer Id.", "Settlement of Cash invoice ", 
"If Customer payment is not done Does this Sale falls under tempory credit sale? (Yes/No)",
"Has the duely completed tempory credit application form been attached",
"If this sale is on Tempory credit sale Has a gate pass been issued?",
"Has Invoice items been delivered physically?(checked issued seal and security log book)",
"Checked arithmetical accuracy (Calculation accuracy)"];
...
function loadData() {
  ...
  xhr.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
     ...
     for (let i = 0; i < 10; i++) {
  for (let j = 0; j < 9; j++) {
    document.getElementById((i * 9) + j + 1).value = data[i][j];
  }
}

    }
  };
  xhr.open("GET", "get/getdata5.php?date=" + site +"&site="+site1, true);
  xhr.send();
}
</script>
</body>
